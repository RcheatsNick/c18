...
module.exports = class BlacklistUserCommand extends Command {
	...
	async run(msg, args) {
			if (args.string === "evet") {
				...
				return msg.channel.send(`<:check:509661885843505153> Reklam engelleme özelliği: **açık**.`);
			}
			if (args.string === "hayır") {
				const vt = this.client.provider.get(msg.guild.id, 'reklamEngel', []);
				this.client.provider.set(msg.guild.id, 'reklamEngel', false);
				return msg.channel.send(`<:check:509661885843505153> Reklam engelleme özelliği: **kapalı**.`);
			}
	}
};