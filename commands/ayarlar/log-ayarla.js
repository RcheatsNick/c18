	...
	async run(msg, args) {
        ...
        if (args.channel) {
			...
			if (vt === args.channel.id) {
				...
				msg.channel.send(`<:xx:509661885973397504> Log kanalı zaten **${args.channel.name}** olarak ayarlı.`);
			} else {
				this.client.provider.set(msg.guild.id, 'logsChannel', args.channel.id);
				this.client.provider.set(msg.guild.id, 'logsEnable', true);
				return msg.channel.send(`<:check:509661885843505153> Log olarak ayarlanan kanal: **${args.channel.name}**`);
			}
        }
    }
};